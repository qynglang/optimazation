...
import numpy as np
import pdb
#This is a script using CRO algorithm to find the Global minimum of Rastrigin Function.
#The new fitness function is commented.
POPSIZE = 25
KE_LOSSRATE = 0.8
MOLEC_COLLRATE = 0.2
KE_INITIAL = 1000000
...
for i in range (0,NUMBER_OF_DIMENSION):
        m.append(pow(2,NUMBER_OF_DIMENSION-i-1))        
m=np.array(m)
#The weight to attach transfering binary strings into real number
class M(object):
...
def fitness(omega):
        ...
        s=n.dot(m)
        s=s.sum()
        #Change binary string into real bin number.
        x=-5.12+10.24*s/255
        #caculate x with the bin number.
        f=10+pow(x,2)-10*np.cos(2*np.pi*x)
        #One dimentional Rastrigin Function.
        return f
